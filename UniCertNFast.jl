...
# Tools for mixed radix
# Array of invmod(p_j ,p_i) for j,i
function invmod_list(p::Array{fmpz, 1})
P = []#Array{Array{fmpz, 1},1}[]    
    for i= 1: length(p)
        Q = []#Array{fmpz, i-1}
        for j = 1: i-1
             push!(Q, invmod(p[j], p[i]))         
        end
          push!(P, Q)
    end
    return P
end
# Array of prod(p_1...p_i) 
function prod_p_list(p::Array{fmpz, 1})
P = Array(ZZ, length(p))
P[1] = fmpz(1)
    for i = 2: length(p)
        P[i] = P[i-1]*p[i-1]
...
# given x mod p_1 ... x mod p_r, find x mod p
############################################
#       convert: Mixed radix base extension
############################################


function mixed_radix(R::RNS, a::RNSmat)
A = Generic.MatSpaceElem{NfAbsOrdElem{AnticNumberField,nf_elem}}[]
    for i=1:length(a.x)
        y = a.x[i]
        for j=1:i-1
             y = R.Pi[i][j]*(y-imageM(R.F[i][2], A[j]))
        end
        push!(A, R.P[i]*preimageM(R.F[i][2],y))
    end
    return A
...
# Given an lower bound "start ", totally split primes are generated to achive the product "bound"
...
#= example
include("/home/ajantha/Documents/RNS/UniCertNFastGit.jl")
Zx,x=FlintZZ["x"]
k,a=number_field(x^3+7x+1)
A=bad_mat(k,15,-100:100);
@time UniCertNF(A, 100, 100000)
